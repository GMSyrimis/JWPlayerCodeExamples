...
public class PlaylistArrayAdapter extends ArrayAdapter<PlaylistItem> {
    ...
    // Width is same as device width, height is 16:9
    private int playerRowPortraitWidth;
    private int playerRowPortraitHeight;


    public PlaylistArrayAdapter(Context context, List<PlaylistItem> objects, Typeface font) {
        super(context, 0, objects);
        this.mFont = font;
        this.mContext = context;
    }

    public PlaylistArrayAdapter(Context context, List<PlaylistItem> objects) {
        super(context, 0, objects);
        this.mContext = context;
    }
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        devicePortraitWidth = displayMetrics.widthPixels;
        devicePortraitHeight = displayMetrics.heightPixels;



        // Define player row height and width
        int px = Math.round(TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 40, displayMetrics));

        playerRowPortraitWidth = devicePortraitWidth - px;
        playerRowPortraitHeight = (playerRowPortraitWidth * 9) / 16;
